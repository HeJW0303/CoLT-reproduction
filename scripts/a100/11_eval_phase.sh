...
bash "$REPO_ROOT/scripts/a100/09_download_eval_data.sh" "$phase"
...
for index in "${!datasets[@]}"; do
  ...
  bash "$REPO_ROOT/scripts/a100/eval_one.sh" "$dataset" "$gpu" &
  pids+=("$!")
...
python "$REPO_ROOT/scripts/a100/summarize_colt_eval.py" "$EVAL_OUTPUT_ROOT/codefaithful"
exit "$status"
