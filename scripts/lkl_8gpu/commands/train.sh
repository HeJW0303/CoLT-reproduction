...
cmd_train() {
  ...
  case "$target" in
    ...
    oracle-k)
      ...
      export COLT_ORACLE_K_REQUIRE_PREOPTIMIZER_INIT="${COLT_ORACLE_K_REQUIRE_PREOPTIMIZER_INIT:-$((1 - resume))}"
  ...
  if [[ "$target" == oracle-k ]]; then
    ...
    [[ "$COLT_ORACLE_K_REQUIRE_PREOPTIMIZER_INIT" == 0 || "$COLT_ORACLE_K_REQUIRE_PREOPTIMIZER_INIT" == 1 ]] || \
      die "COLT_ORACLE_K_REQUIRE_PREOPTIMIZER_INIT must be 0 or 1"
  ...
  cp "$REPO_ROOT/transformers-4.57.0/src/transformers/models/qwen3_vl/modeling_colt_latent_heads.py" "$run_record/"
  ...
  python -m pip freeze > "$run_record/pip_freeze.txt"
  {
    ...
    printf 'COLT_ANSWER_VISIBILITY=%s\n' "${COLT_ANSWER_VISIBILITY:-full}"
    printf 'COLT_STOCHASTIC_LATENT=%s\n' "${COLT_STOCHASTIC_LATENT:-0}"
    printf 'COLT_LARE_REFOCUS=%s\n' "${COLT_LARE_REFOCUS:-0}"
    printf 'COLT_LARE_DIM=%s\n' "${COLT_LARE_DIM:-1536}"
    printf 'COLT_LARE_LAYERS=%s\n' "${COLT_LARE_LAYERS:-2}"
    printf 'COLT_LARE_HEADS=%s\n' "${COLT_LARE_HEADS:-12}"
    printf 'COLT_LARE_QUERIES=%s\n' "${COLT_LARE_QUERIES:-4}"
    printf 'COLT_LARE_MAX_STEPS=%s\n' "${COLT_LARE_MAX_STEPS:-16}"
    printf 'COLT_LARE_DROPOUT=%s\n' "${COLT_LARE_DROPOUT:-0.0}"
    ...
    printf 'COLT_ORACLE_K_REQUIRE_PREOPTIMIZER_INIT=%s\n' "${COLT_ORACLE_K_REQUIRE_PREOPTIMIZER_INIT:-0}"
