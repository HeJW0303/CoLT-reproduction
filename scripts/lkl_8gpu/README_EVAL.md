...
```bash
cd /data/nvme0/lkl/work/CoLT-reproduction
...
```bash
CUDA_VISIBLE_DEVICES=6 bash scripts/lkl_8gpu/10_eval_smoke.sh
```
...
无界面 A100/A800 服务器固定使用 `opencv-python-headless==4.11.0.86`，避免普通
...
cd /data/nvme0/lkl/work/CoLT-reproduction
bash scripts/lkl_8gpu/11_eval_phase.sh phase1
```

如果要先用全部 8 张 GPU 对完整 MMStar 做数据并行评估：

```bash
bash scripts/lkl_8gpu/12_eval_mmstar_8gpu.sh
```
...
bash scripts/lkl_8gpu/13_eval_remaining_8gpu.sh
```
...
评估环境启用原子写入：rank pickle、最终 xlsx/csv 和首次解码的图片都会先写入
...
- 只加载 `/data/nvme0/lkl/models/Qwen3-VL-8B-Instruct`；
...
```bash
bash scripts/lkl_8gpu/11_eval_phase.sh phase2
```
...
```bash
bash scripts/lkl_8gpu/11_eval_phase.sh phase3
```
...
```bash
COLT_EVAL_GPUS=3,4,5 bash scripts/lkl_8gpu/11_eval_phase.sh phase1
```
...
```bash
bash scripts/lkl_8gpu/09_download_eval_data.sh phase1
bash scripts/lkl_8gpu/09_download_eval_data.sh phase2
bash scripts/lkl_8gpu/09_download_eval_data.sh phase3
```
