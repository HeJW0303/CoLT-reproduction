...
class RolloutConfig:
    ...
    def post_init(self):
        if self.name not in {"vllm", "colt_transformers"}:
        ...
        if self.name == "colt_transformers":
            ...
            if self.ignore_eos:
                raise ValueError("colt_transformers rollout does not support ignore_eos=True.")
            if not 0.0 < self.top_p <= 1.0:
                raise ValueError("colt_transformers rollout requires top_p in (0, 1].")
