...
class FSDPWorker(Worker):
    ...
    def _build_rollout(self) -> None:
        if self.config.rollout.name == "colt_transformers":
            from .rollout.colt_transformers_rollout import CoLTTransformersRollout
            from .sharding_manager.fsdp_transformers import FSDPTransformersShardingManager
            ...
            self.rollout_sharding_manager = FSDPTransformersShardingManager(
                module=self.fsdp_module,
                inference_model=self.rollout.inference_model,
                use_param_offload=self._use_param_offload,
                seed=self.config.rollout.seed,
            )
            print_gpu_memory_usage("After CoLT Transformers rollout init")
            return

        if self.config.rollout.name != "vllm":
            raise ValueError(f"Unsupported rollout backend: {self.config.rollout.name}")
    ...
    def init_model(self):
        if self.config.rollout.name == "colt_transformers":
            ...
            os.environ["COLT_RL_TOKENIZER_PATH"] = self.config.actor.model.model_path
    ...
    def prepare_rollout_engine(self):
        self.rollout_sharding_manager.prepare_rollout()

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def release_rollout_engine(self):
        self.rollout_sharding_manager.release_rollout()
