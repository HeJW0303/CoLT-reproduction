            ...
            with timer("step", timing_raw):
                ...
                with timer("old", timing_raw):
                    ...
                    if self.config.worker.rollout.name == "colt_transformers":
                        ...
                        valid_differences = torch.abs(rollout_log_probs - recomputed_log_probs)[response_mask]
                        if valid_differences.numel() == 0:
                            raise RuntimeError("CoLT rollout produced no valid response tokens for log-prob parity check.")
                        max_logprob_difference = float(valid_differences.max().item())
                        metrics["policy/rollout_logprob_max_abs_diff"] = max_logprob_difference
                        if not np.isfinite(max_logprob_difference) or max_logprob_difference > 5e-2:
                            raise RuntimeError(
                                "CoLT rollout/actor log-prob mismatch exceeds tolerance: "
                                f"max_abs_diff={max_logprob_difference:.6g}, tolerance=0.05"
                            )
